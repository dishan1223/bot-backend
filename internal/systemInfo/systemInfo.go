...
func GetModelDeps(msg string, lat string, lon string) (string, error) {
	...
	if containsAny(msg, consts.TIME_KEYWORDS) {
		// when i hosted the server on Render, the model was not responding bangladeshi time
		// LoadLocation("asia/dhaka") sets the location to bangladesh and then we get accurate data
		// for Bangladeshi time
		...
		bangladeshiTime := time.Now().In(loc)

		system_info = bangladeshiTime.Format(time.RFC3339)
	}
